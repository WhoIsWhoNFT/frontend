...
const Box: React.FC<{ style: any; item: any; index: number; img?: string }> = ({
    style,
    item,
    index,
    img,
    ...
    return (
        <BoxItem style={{ ...style }}>
            <Portrait image={item.pic} style={{ backgroundImage: item.css }} />
            <BoxDesc>
                <div
                    id={'team_N' + index}
                    style={{
...
export default function App() {
    ...
    useChain([introAPI, springApi, transApi], [0.1, 0.5, 1]);
                    ...
                    <Content>
                        <BoxesContainer>
                            {transition((style, item, state, index) => (
                                <div
                                    className={
                                        activeProfile === index ? 'BoxItemA' : 'BoxItem'
                                    }
